...
cs_locale(CS_CONTEXT * ctx, CS_INT action, CS_LOCALE * locale, CS_INT type, CS_VOID * buffer, CS_INT buflen, CS_INT * outlen)
{
	...
	if (action == CS_SET) {
		...
		case CS_SYB_CHARSET:
			if (buflen == CS_NULLTERM) {
				buflen = (CS_INT)strlen((char *)buffer);
			}
	...
	else if (action == CS_GET)
	{
		...
		switch (type) {
		case CS_SYB_CHARSET:
			tlen = (int)((locale->charset ? strlen(locale->charset) : 0) + 1);
			if (buflen < tlen)
			{
				if (outlen)
					*outlen = tlen;
		...
		case CS_SYB_LANG:
			tlen = (int)((locale->language ? strlen(locale->language) : 0) + 1);
			if (buflen < tlen)
			{
				if (outlen)
					*outlen = tlen;
		...
		case CS_SYB_LANG_CHARSET:
		{
			...
			tlen = (int)((locale->language ? strlen(locale->language) : 0) + 1);
			clen = (int)((locale->charset ? strlen(locale->charset) : 0) + 1);
		...
		case CS_SYB_SORTORDER:
			tlen = (int)((locale->collate ? strlen(locale->collate) : 0) + 1);
			if (buflen < tlen)
			{
				if (outlen)
					*outlen = tlen;
